...
<script>
  ...
  function loginnow() {
	  FB.login(function(response) {
		...
		}, {perms:'email,sms,user_interests,user_likes'});
  }
  function logoutnow() {
	var access_token = "";
	FB.login(function (response) {
	...
	alert(access_token);
	var mylikes = "/me/likes?access_token=" + access_token;
	//alert(mylikes);
	document.getElementById('Form_Form_first_name').value = mylikes;
	FB.api(mylikes, function(responset) {
		for (var i=0, l=responset.data.length; i<l; i++) {
			var like = responset.data[i];
			alert(objectToString(like));
		}
		//alert(responset);
		//alert(objectToString(responset));
	});
  }
...
<div style="margin-left: 20px;"><img src="http://imu.uiowa.edu/assets/stayupdated.png" /><!--$ThemeDir/images/stayupdated.png-->
